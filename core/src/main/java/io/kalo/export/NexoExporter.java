...
/**
 * Exports Kalo content to Nexo format.
 * 
 * Note: This is a lossy conversion. Kalo features that don't have
 * Nexo equivalents will be flagged as unsupported.
 */
public class NexoExporter implements Exporter {
    ...
    public ExportReport export(Path source, Path target) throws Exception {
        ...
        try (Stream<Path> paths = Files.walk(source)) {
            paths.filter(Files::isRegularFile)
                 ...
                 .forEach(path -> {
                     try {
                         convertToNexo(path, source, target, report);
                     } catch (Exception e) {
    ...
    private void convertToNexo(Path sourceFile, Path sourceRoot, Path targetRoot, ExportReport report) throws IOException {
    ...
    private void convertItem(Path source, Path target, ExportReport report) throws IOException {
        ...
        report.partial(fileName, "Features may not have Nexo equivalents");
    }
    ...
    private void convertBlock(Path source, Path target, ExportReport report) throws IOException {
        ...
        // 3. Handle virtual blocks (may not be possible in Nexo)
        // 4. Report unsupported features
        ...
        String fileName = source.getFileName().toString();
        report.exported(fileName);
        report.partial(fileName, "Virtual blocks not supported in Nexo");
    }
