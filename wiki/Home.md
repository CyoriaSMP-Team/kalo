# 🐈 Kalo Wiki

**Open Custom Content Engine for Minecraft**

Build once. Play everywhere. No paywalls. No player caps.

---

## 🚀 Quick Start

- [[Getting-Started]] — Install and configure Kalo
- [[First-Pack]] — Create your first content pack

## 📦 Content Types

- [[Items]] — Custom items with models and textures
- [[Blocks]] — Custom blocks using vanilla states
- [[Furniture]] — Rotatable furniture with seats and storage
- [[Armor]] — Custom armor with equipment textures
- [[Paintings]] — Custom paintings
- [[Music-Discs]] — Custom jukebox-playable discs
- [[GUIs]] — Server-side inventory menus
- [[Sounds]] — Custom sound events
- [[Glyphs]] — Custom font characters
- [[Recipes]] — Crafting and smelting recipes

## 🌏 Platform

- [[Bedrock]] — Bedrock Edition support
- [[Folia]] — Folia server support

## 🔄 Migration

- [[Migration]] — Import from Oraxen, ItemsAdder, CraftEngine, Neko

## 📖 Reference

- [[Configuration]] — All config options
- [[Commands]] — Available commands
- [[Permissions]] — Permission nodes
- [[API]] — Developer API reference

## 🔗 Links

- [GitHub Repository](https://github.com/CyoriaSMP-Team/kalo)
- [Issues](https://github.com/CyoriaSMP-Team/kalo/issues)
- [Discord](https://discord.gg/kalo) (coming soon)

---

> **Status:** Pre-alpha (v0.1.0)
> **License:** MIT
> **Java:** 21+ (Paper 1.21.4) / 25 (Paper 26.2)

---

## 📜 Footer

**Kalo** — Open Custom Content Engine for Minecraft

[GitHub](https://github.com/CyoriaSMP-Team/kalo) • [Issues](https://github.com/CyoriaSMP-Team/kalo/issues) • [Discord](https://discord.gg/kalo) (coming soon)

> **Version:** 0.1.0 • **License:** MIT • **Java:** 21+ / 25

*Build once. Play everywhere.*
